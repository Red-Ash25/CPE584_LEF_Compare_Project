...
# https://github.com/Red-Ash25/CPE584_LEF_Compare_Project
#
...
class LibRuleChecker
  def self.check_pin_value_in_lef(lib_path, lef_path, lef_pin, lib_pin, lib_pin_prop_key, cell)
    ...
    # Return empty if property doesn't exist in LIB
    if lib_pin_prop.nil?
      return errors
    else 
    ...
    # Skip if LEF does not define the property
    unless lef_has_key
      return errors
    end
    ...
    when "DIRECTION"
      ...
      unless lef_dir == lib_dir
        ...
        errors << "DIRECTION mismatch — LIB=#{lib_dir}, LEF=#{lef_dir}"
      end

    when "CLOCK"
      puts "CELL=#{cell} PIN=#{lib_pin_name} — Checking CLOCK"
      ...
      if lib_clock_val == "TRUE"
        ...
        unless valid_clock_uses.include?(lef_use_val)
          ...
          errors << "CLOCK mismatch — LIB clock:true but LEF USE=#{lef_use_val} (expected CLOCK or SIGNAL)"
        end
        ...
        if lef_use_val == "CLOCK"
          ...
          errors << "CLOCK mismatch — LIB clock:false but LEF USE=CLOCK"
        end
      end
    ...
    when "PG_TYPE"
      ...
      when "PRIMARY_POWER"
        ...
        unless lef_use_val == "POWER"
          ...
          errors << "PG_TYPE mismatch — LIB PRIMARY_POWER but LEF USE=#{lef_use_val} (expected POWER)"
        end
      when "PRIMARY_GROUND"
        # Must be USE GROUND
        unless lef_use_val == "GROUND"
          puts "  ERROR: PG_TYPE mismatch LIB=PRIMARY_GROUND LEF=#{lef_use_val}"
          errors << "PG_TYPE mismatch — LIB PRIMARY_GROUND but LEF USE=#{lef_use_val} (expected GROUND)"
        end
      else
        ...
        if ["POWER", "GROUND"].include?(lef_use_val)
          ...
          errors << "PG_TYPE mismatch — LIB non-power pin but LEF USE=#{lef_use_val}"
        end
      end

    when "VOLTAGE_NAME"
      puts "CELL=#{cell} PIN=#{lib_pin_name} — Checking VOLTAGE_NAME"
      
      lib_voltage_name = lib_pin.property("voltage_name")
      lib_voltage_name = lib_voltage_name.to_s.gsub(/[\";]/, "").strip.upcase
      
      unless lib_voltage_name == lib_pin_name
        puts "  ERROR: VOLTAGE_NAME mismatch"
        errors << "VOLTAGE_NAME mismatch — voltage_name=#{lib_voltage_name}"
      end

    when "RELATED_POWER_PIN"
      puts "CELL=#{cell} PIN=#{lib_pin_name} — Checking RELATED_POWER_PIN"
      
      ref_name = lib_pin_prop.upcase
      unless ref_name.match?(/^(VPWR|VPB|VDD|VDDIO|VCC)/)
        puts "  ERROR: related_power_pin '#{ref_name}' doesn't look like a power pin"
        errors << "related_power_pin '#{ref_name}' does not look like a power pin name"
      end

    when "RELATED_GROUND_PIN"
      puts "CELL=#{cell} PIN=#{lib_pin_name} — Checking RELATED_GROUND_PIN"
      
      ref_name = lib_pin_prop.upcase
      unless ref_name.match?(/^(VGND|VNB|VSS)/)
        puts "  ERROR: related_ground_pin '#{ref_name}' doesn't look like a ground pin"
        errors << "related_ground_pin '#{ref_name}' does not look like a ground pin name"
      end
    end
...
def compare_lef_lib(parsed_lef_files, liberty_data, errors)
    ...
    # CRITICAL: Convert the Hash structure to Array of formatted strings
    if errors[:liberty_incorrect_pin_property].is_a?(Hash)
      ...
      errors[:liberty_incorrect_pin_property].keys.sort.each do |cell|
        ...
        errors[:liberty_incorrect_pin_property][cell].keys.sort.each do |pin|
          ...
          # One more level (property => [messages])
...
# (COMPLETED: Deleted def check_for_uncommon_properties as stated above)
