...
class LibRuleChecker
  def self.check_pin_value_in_lef(lib_path, lef_path, lef_pin, lib_pin, lib_pin_prop_key, cell)
    ...
    puts "LEF PIN PROPERTIES: #{lef_pin.properties.inspect}"
    ...
    # ===== DEBUG: pin/property comparison =====
    puts "----------------------------------------"
    puts "CELL        : #{cell}"
    puts "PIN         : #{lib_pin.name}"
    puts "PROP KEY    : #{lib_pin_prop_key}"
    puts "----------------------------------------"
    ...
    when "DIRECTION"
     puts "CHECKING Direction:  #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      ...
      puts "lef_val #{lef_val}"
      puts "lib_val #{lib_val}" 
      unless lef_val == lib_val
        puts "  ERROR: Direction mismatch LIB=#{lib_val}, LEF=#{lef_val}"
        errors << "#{cell}\n\tPin #{pin_name}: DIRECTION mismatch — LIB=#{lib_val}, LEF=#{lef_val}\n"
      end
    ...
    when "CLOCK"
      ...
      puts "lef_use #{lef_val}"
      puts "lib_val #{lib_val}" 

      if lib_val == "TRUE" && lef_use != "CLOCK"
        errors << "#{cell}\n\tPin #{pin_name}: CLOCK mismatch — LIB clock=true but LEF USE=#{lef_use}\n"
      elsif lib_val == "FALSE" && lef_use == "CLOCK"
        errors << "#{cell}\n\tPin #{pin_name}: CLOCK mismatch — LIB clock=false but LEF USE=CLOCK\n"
      end
    # PG_TYPE
    when "PG_TYPE"
      puts "CHECKING PG_type: #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      lef_use_lines = lef_pin.properties.select do |l|
        l.strip.upcase.start_with?("USE")
      end
      ...
      when "PRIMARY_POWER"
        unless lef_use == "POWER"
          puts "  ERROR: PG_TYPE mismatch LIB=PRIMARY_POWER LEF=#{lef_use}"
          errors << "#{cell}\n\tPin #{pin_name}: PG_TYPE mismatch — LIB=PRIMARY_POWER but LEF USE=#{lef_use} (expected POWER)\n"
        end
      when "PRIMARY_GROUND"
        unless lef_use == "GROUND"
          puts "  ERROR: PG_TYPE mismatch LIB=PRIMARY_GROUND LEF=#{lef_use}"
          errors << "#{cell}\n\tPin #{pin_name}: PG_TYPE mismatch — LIB=PRIMARY_GROUND but LEF USE=#{lef_use} (expected GROUND)\n"
        end
        
    # VOLTAGE_NAME
    when "VOLTAGE_NAME"
      puts "CHECKING Voltage_NAME: #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      lib_pin_name = lib_pin_name.to_s.strip.upcase
    ...
    when "RELATED_POWER_PIN"
      puts "CHECKING RELATED_PIN_POWER: #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      puts "DBG ENTER #{cell} #{lib_pin.name} #{prop_key} val=#{lib_val.inspect}"
      #puts "CHECKING RELATED_POWER_PIN: #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      ...
      puts "DBG #{cell} #{pin_name} #{prop_key}=#{lib_val}"
      # RELATED_GROUND_PIN 
    when "RELATED_GROUND_PIN"
      puts "CHECKING RELATED_GROUND_POWER: #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      puts "DBG ENTER #{cell} #{lib_pin.name} #{prop_key} val=#{lib_val.inspect}"

      #puts "CHECKING RELATED_GROUND_PIN: #{cell} #{lib_pin.name} #{lib_pin_prop_key}"
      ref_name = lib_val.upcase
      puts "DBG #{cell} #{pin_name} #{prop_key}=#{lib_val}"
...
=begin
        puts "\n================ PARSED LIBERTY CELL ================"
        puts "LIB FILE : #{filename}"
        puts "CELL     : #{next_liberty_cell.name}"
        puts "  Pins:"

        next_liberty_cell.pins.each do |pin|
          puts "    #{pin.name}:"

          props = pin.searchedProperties || []
          props.each do |prop|
            val = pin.property(prop)
            puts "      #{prop} = #{val.inspect}"
          end
        end
        puts "===================================================="
