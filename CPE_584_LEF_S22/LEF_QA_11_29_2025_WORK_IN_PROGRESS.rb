...
def parse_lef_files(opts, lef_files, tlef_files, errors)
  ...
  reportDirectoryExists = nil # Flag for the creation of comment file directory.
  # Creating a name for the comment directory, if creation of such directory is needed, based on timestamp.
  # Example directory name: LEFQA_Report-11_29_2020:17:45:32
  ...
  lef_files.each { |lef_file_path|
    ...
    begin
      # Warning: User must have permission to create comment file in the directory.
      if !comment_lines.empty? 
        # Creates comment file
    ...
    # If user does not have permission to create comment file, create a directory within the user's home and place comment files there.
    rescue Errno::EACCES => e
      puts "Can't create LEF comment file in #{comments_filename}. No permission."

      # If comment file directory has already been created, place comment file there.
      ...
      # If there is not a directory for the placement of comment files, create one in user's home.
      if !reportDirectoryExists
      puts "Creating Directory #{reportDirectoryName} in user's home directory." 

      # Create directory in user's home "~" directory to hold comment files.
      # According to specific operating systems "~" below may need to be replaced with "ENV['HOME']"
      reportDirectoryCommand = "mkdir ~/'#{reportDirectoryName}'" 
      system(reportDirectoryCommand)
      system(reportDirectoryCommand + "/commentFiles")
      # Set comment directory flag after its creation to prevent creating a directory for each comment file.
      reportDirectoryExists = true

      # Place file into new directory, now that it has been created.
      puts "Placing comment file in the report directory within user's home."
        comments_filename = ENV['HOME'] + "/" + reportDirectoryName + "/commentFiles/" + File.basename(comments_filename)
        comments_file = File.open(comments_filename, "w")
          comment_lines.each do |line|
  ...
  unless liberty_dirpath.nil? && liberty_files.nil?
    ...
    liberty_files.each do |filename|
      ...
      while !(cell_lines_list.empty?)
        #puts "cell_lines_list:"
        #puts cell_lines_list
        #puts "cell properties lists"
        #puts cell_properties_lists
        # puts "pin lines list"
        # puts pin_lines_list
...
def compare_lef_lib(parsed_lef_files, liberty_data, errors)
    ...
    parsed_lef_files.each_pair { |lef_filename, parsed_lef_file|
	  ...
	  print "Comparing LEF[#{lefcount}/#{parsed_lef_files.length}]='" + File.basename(lef_filename) + "'  to  LIB='" + File.basename(filename) + "'                                                   \r"
        ...
        end #end liberty_data.each
      end #end parsed_lef_files.cells()
      lefcount += 1 
    } #end parsed_lef_files
    puts "" 
    
    if !(missing_cells.empty?) #There are missing cells
    ...
    if !(area_mismatch.empty?) #There is area mismatch
    ...
    if !(missing_pins.empty?) #There are missing pins
  ...
  def initialize(cell_start_lines, cell_properties, pin_start_lines, pin_properties)
    ...
    Liberty_Cell::properties().each do |property|
      ...
      if !(cell_properties[property].empty?)
         ...
         #puts "Line #=#{cellpropline[0]} prop=#{cellpropline[1]} value=#{cellpropline[2].chomp(';')}"
        if cellpropline[0].to_i() < next_cell_start_line_num 
          @properties[property] = cell_properties[property].shift().split(': ')[1]  #returns property name
          #puts @properties[property]
        end
      end
    end
      if !(pin_start_lines.empty?)
      #puts pin_start_lines[0].split(' ')[0].to_i()
...
class DdcScanner
  ...
  def self.scan_for_dirs(proj_dir)
    ...
    # Warning: Lines containing -maxdepth may need numerical value changed depending on file architecture.
    # Suggestion: Create command line argument flag to switch between file architectures.
    find_pnr_cmd = [
      "find",
      "#{proj_dir}",
      "-maxdepth 3",
    ...
    #Find and check for config, pnr,and syn directories
    ...
    ccount = pcount = scount = 0 #counter for filetypes
    ...
    if ccount < 1 || pcount < 1
      exit
    end    # config and pnr directories must be present, otherwise script exits.
  ...
  #
  # Can this search be streamlined with -maxdepth ?
...
def ddc_scan_from_sysio(proj_dir)
  ...
  option_choice = "1" #Defaults to first DDC
  if count > 1 #Number of DDC found
    #Multiple DDC found
    puts "\nMultiple DDC found. Please select one of the following:\n"
    puts ddc_output #Lists available/found DDC for user to chose from
    loop do
      print "Select DDC [1-#{count}]:"
      option_choice = gets.strip #Recieve Input from user
      break if (option_choice.to_i <= count) & (option_choice.to_i >= 1)
      puts "Invalid option, please select a number between [1-#{count}]"
    end
  else
    #One DDC directory found, uses default option_choice
...
if __FILE__ == $0
  ...
  begin
    ...
    parser = OptionParser.new do |o|
      o.separator "Description: This script sorts and compares LEF files to LIB files. The user will be prompted to choose the running DDC and LIB dir while the script runs."
      o.separator "             Script must either be run at the top of technology library, or passed a library using the -w option (config, pnr, and syn dirs must be seen)"
			o.separator "                    LEF files and LIB dirs are found in working dir under pnr and syn dirs respectively. Can instead pass either or both as an option."
			o.separator "             Script uses the first TLEF file found. To specify TLEF file, use -t option. If no TLEF files are found, default layers are are used."
			o.separator "             Output files are placed in the same dir as assocated LEF files. If user does not have permission to write there, a dir will be created in ~/LEFQA_Report-{Current date and time}."
			o.separator "Options:"
      ...
      o.on("-s", "Only sort LEF File. Needs to be last option, or results in error. Cannot be run with -w option. May take a while to complete running.") do |lef|
        lef = ARGV[-1]
