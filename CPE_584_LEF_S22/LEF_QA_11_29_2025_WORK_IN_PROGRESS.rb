  ...
  def check_via_obs_association(errors)
    ...
    if @obstructions.nil?
      pin_cut_layers.each do |cut_layer, pin_info_list|
        pin_info_list.each do |pin_info|
          errors[:missing_via_obs] << "Line #{pin_info[:line_num]}: Cell #{@name}, pin #{pin_info[:pin_name]} has cut layer #{cut_layer} but cell has no OBS section\n"
        end
      end
      return
    end
    ...
    pin_cut_layers.each do |cut_layer, pin_info_list|
      ...
      unless obs_layer_names.include?(normalized_cut)
        pin_info_list.each do |pin_info|
          errors[:missing_via_obs] << "Line #{pin_info[:line_num]}: Cell #{@name}, pin #{pin_info[:pin_name]} - cut layer #{cut_layer} missing from OBS\n"
        end
      end
    end
  end
...
class LayerCollection
  ...
  # Fallback patterns - ONLY used if no TLEF is provided
